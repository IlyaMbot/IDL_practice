...
level = 000 # setting up the lever for flares' searching
percent = 40 #same in percents

upper_val = 4000 # the highest value if image's intensity is too high 
...
def general_plot(image, loop1 = None, loop2 = None):
    ...
    ax[0].plot(
        [5  + pixel11[1], 32 + pixel12[1]],
        [25 + pixel11[0], 6  + pixel12[0]],
        'wx' ,ls = '-', fillstyle='none', markersize=15
        )
    length1 = int(np.sqrt(((25 + pixel11[0]) - (  6 + pixel12[0]))**2 + ((5  + pixel11[1]) - (32 + pixel12[1]))**2)*resolution)
    ax[0].set_title("distance ~ {}\'\' ".format(length1), pad=30)
    ...
    ax[1].plot(
        [5  + pixel21[1], 32 + pixel22[1]],
        [25 + pixel21[0], 6  + pixel22[0]],
        'wx' , ls = '-', fillstyle='none', markersize=15
        )
    length2 = int(np.sqrt((25 + pixel21[0] - (6  + pixel22[0] ))**2 + ( 5  + pixel21[1] - (32 + pixel22[1]) )**2)*resolution)
    ax[1].set_title("distance ~ {}\'\' ".format(length2), pad=30)

    fig.colorbar(pmc, ax=ax[1])

        
    plt.tight_layout()
    plt.savefig('/home/conpucter/GitHub/IDL_practice/ind/result.png', transparent=False, dpi=880, bbox_inches="tight")
    plt.show()
